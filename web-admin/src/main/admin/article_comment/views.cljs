    ...
    (fn []
      [:form
       ...
       [:div {:class "flex justify-center items-center space-x-4 mt-4"}
        [btn-new {:on-click #(re-frame/dispatch [:add-article-comment @edit])}
         "Save"]]])))

(defn edit-form []
  (let [article-comment (re-frame/subscribe [:article-comment/edit])]
    (fn []
      (when-let [edit (r/atom @article-comment)]
        [:form
         [:div {:class "grid gap-4 mb-4 sm:grid-cols-2"}
          (text-input-backend {:label "Name"
                               :name "name"
                               :default-value (:name @article-comment)
                               :on-change #(swap! edit assoc-in [:name] (f-util/get-value %))})
          (text-input-backend {:label "Description"
                               :name "descrtiption"
                               :default-value (:description @article-comment)
                               :on-change #(swap! edit assoc-in [:description] (f-util/get-value %))})]
         [:div {:class "flex justify-center items-center space-x-4"}
          [btn-new {:on-click #(re-frame/dispatch [:update-article-comment @edit])}
           "Save"]]]))))

(defn delete-form []
  (let [current (re-frame/subscribe [:article-comment/edit])]
    (fn []
      (when @current
        [:form
         ...
         [:div {:class "flex justify-center items-center space-x-4"}
          [red-button {:on-click #(re-frame/dispatch [:delete-article-comment (:id @current)])}
           "Delete"]]]))))

(defn action-fn [e]
  [:div
   [btn-edit {:on-click #(re-frame/dispatch [:get-article-comment
                                             (:id e)
                                             [[:dispatch [:set-modal {:show? true
                                                                      :title "article-comment Edit"
                                                                      :child edit-form}]]]])}
    "Edit"]
   [:span {:class css/divi} "|"]
   [btn-del {:on-click #(re-frame/dispatch [:get-article-comment
                                            (:id e)
                                            [[:dispatch [:set-modal {:show? true
                                                                     :title "article-comment Delete"
                                                                     :child delete-form}]]]])}
    "Del"]])
      ...
      (let [q-data (r/atom (assoc @default-pagination
        ...
        [:form
         ...
         [:div {:class "felx inline-flex justify-center items-center w-full"}
          [btn-query {:on-click #(re-frame/dispatch [:query-articles-comments @q-data])} "Query"]
          [btn-new {:on-click #(re-frame/dispatch [:set-modal {:show? true
                                                               :title "article-comment Add"
                                                               :child new-form}])
                    :class css/button-green} "New"]]]))))

(defn data-table []
  (let [datasources (re-frame/subscribe [:article-comment/datasources])]
    (fn []
